...
def get_progress_dict_filename(uid,cycle,username,default_dir):
    ...
    """
    if 'proposal' in db[uid].v2.start.keys():
        run = db[uid].v2.start
        fn = '%s%s/pass-%s/Results/%s/progress_dict_%s.json'%(default_dir,run['cycle'],run['proposal']['proposal_id'],run['uid'],run['uid'])
    else:
        fn = default_dir+'%s/%s/Results/%s/progress_dict_%s.json'%(cycle,username,uid,uid)
    return fn
...
def visualize_processing_overview_dict(processing_overview_dict_file,max_completed=10,max_statistic=20,current_timeout=None,complete_timeout=None,update_time=None,verbose=False):
    ...
    for r in range(int(reps)): # loop for continous updates
        try:
            ...
            tmp = json.load(f);f.close()
            processing_overview_dict=json.loads(tmp);del tmp
            processing_overview_dict_exists = True
            ...
            processing_overview_dict_exists = False
            if verbose:
                print('could not find anything to display....does the processing_overview_dict_file %s exist?'%processing_overview_dict_file)

        ## process_ids previously processed
        if processing_overview_dict_exists and len(list(processing_overview_dict['completed'].keys())) > 0: # i.e. there are actually completed datasets that could be displayed
